...
# One NAT gateway (single AZ) keeps cost down for dev; prod can be extended
# to one-NAT-per-AZ by adding a `nat_high_availability` toggle if desired.
...
# RDS SG: only accepts traffic from ECS tasks - no public access at all
